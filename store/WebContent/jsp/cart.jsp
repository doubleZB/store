					...
					<table class="table table-bordered">
						<tbody>
							...
							<c:if test="${not empty sessionScope.cart.map}">
								<c:forEach items="${cart.map}" var="m"> <!--m.key=pid   m.value=item  --> 
									<tr class="active">
										...
										<td width="20%">
											￥${m.value.product.shop_price}
										</td>
										<td width="10%">
											<input type="text" name="quantity" value="${m.value.count }" maxlength="4" size="10">
										</td>
										<td width="15%">
											<span class="subtotal">￥${m.value.subtotal}</span>
										</td>
										<td>
											<a href="${pageContext.request.contextPath}/cart?method=removeByid&id=${m.key}" class="delete">删除</a>
										</td>
									</tr>
								</c:forEach>
							</c:if>
